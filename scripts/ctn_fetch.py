...
CFG = json.load(open(os.path.join(ROOT, 'config.json'), encoding='utf-8')) if os.path.exists(os.path.join(ROOT, 'config.json')) else {}
UNITS = CFG.get('unidades') or ['CARTAO DE BELEM CENTRO', 'CARTAO DE MANAUS CENTRO', 'CARTAO DE MANAUS NORTE',
         'CARTAO DE OSASCO', 'CARTAO DE PARINTINS', 'CARTAO DE PORTO ALEGRE NORTE']
SUBFR = str(CFG.get('sub_franquia', '5'))
...
def confirm_franchise(r):
    """Tela 'Confirmar Franquia': escolhe a Sub-Franquia do config e a primeira unidade disponível e confirma."""
    url = r.url
    html = r.text
    ...
    data.update({'__EVENTTARGET': 'ctl00$ContentPlaceHolder1$ddlSubFranquia', '__EVENTARGUMENT': '',
                 'ctl00$ContentPlaceHolder1$ddlSubFranquia': SUBFR})
    ...
    fr = next((opts[u] for u in UNITS if u in opts), None) or opts.get('CARTAO DE BELEM CENTRO') or next(iter([v for t, v in opts.items() if t.startswith('CARTAO DE')]), '417')
...
def fetch_xlsx(a, b):
    ...
    # aquela unidade ainda nao vendeu, comum de manha. Sem isto o script morria
    # com IndexError e derrubava a atualizacao (correcao trazida do CDT).
